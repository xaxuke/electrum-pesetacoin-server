...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.pesetacoind_url = 'http://%s:%s@%s:%s/' % (
            config.get('pesetacoind', 'pesetacoind_user'),
            config.get('pesetacoind', 'pesetacoind_password'),
            config.get('pesetacoind', 'pesetacoind_host'),
            config.get('pesetacoind', 'pesetacoind_port'))
    ...
    def do_catch_up(self):
        ...
        self.header = self.block2header(self.pesetacoind('getblock', [self.storage.last_hash]))
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("pesetacoind is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def pesetacoind(self, method, params=[]):
        postdata = dumps({"method": method, 'params': params, 'id': 'jsonrpc'})
        while True:
            try:
                respdata = urllib.urlopen(self.pesetacoind_url, postdata).read()
                break
            except:
                print_log("cannot reach pesetacoind...")
                self.shared.pause()
                time.sleep(10)
    ...
    def get_header(self, height):
        block_hash = self.pesetacoind('getblockhash', [height])
        b = self.pesetacoind('getblock', [block_hash])
    ...
    def hash_header(self, header):
        # OK pesetacoin todo: remove auxpow offsets before hashing
    ...
    def read_chunk(self, index):
        # pesetacoin todo: include auxpow data in chunk...
        with open(self.headers_filename, 'rb') as f:
    ...
    def write_header(self, header, sync=True):
        ...
        # pesetacoin todo: not sure why 40*100 was chosen... half of 80?
    ...
    def flush_headers(self):
        ...
        with open(self.auxpows_filename, 'a+') as f:
            # pesetacoin todo: always write auxpow data at end of file?
    ...
    def get_mempool_transaction(self, txid):
        try:
            raw_tx = self.pesetacoind('getrawtransaction', [txid, 0])
        except:
            return None
    ...
    # pesetacoin todo: not sure if the 80 here is related to block header
    ...
    def import_block(self, block, block_hash, block_height, sync, revert=False):
        ...
        # add undo info
        if not revert:
            self.storage.write_undo_info(block_height, self.pesetacoind_height, undo_info)
    ...
    def process(self, request, cache_only=False):
        ...
        if method == 'blockchain.numblocks.subscribe':
            ...
            if cache_only:
                ...
                #pesetacoin todo: compress result?

        elif method == 'blockchain.transaction.broadcast':
            try:
                txo = self.pesetacoind('sendrawtransaction', params)
            ...
            tx_hash = params[0]
            result = self.pesetacoind('getrawtransaction', [tx_hash, 0])

        elif method == 'blockchain.estimatefee':
            num = int(params[0])
            result = self.pesetacoind('estimatefee', [num])
    ...
    def getfullblock(self, block_hash):
        block = self.pesetacoind('getblock', [block_hash])
        ...
        try:
            ...
            logger.error("pesetacoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run pesetacoind with txindex=1; use -reindex if needed.")
    ...
    def catch_up(self, sync=True):
        ...
        while not self.shared.stopped():
            ...
            # are we done yet?
            info = self.pesetacoind('getinfo')
            self.pesetacoind_height = info.get('blocks')
            pesetacoind_block_hash = self.pesetacoind('getblockhash', [self.pesetacoind_height])
            if self.storage.last_hash == pesetacoind_block_hash:
            ...
            self.up_to_date = False
            try:
                next_block_hash = self.pesetacoind('getblockhash', [self.storage.height + 1])
    ...
    def memorypool_update(self):
        mempool_hashes = set(self.pesetacoind('getrawmempool'))
