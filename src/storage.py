...
from utils import ptc_address_to_hash_160, hash_160_to_pubkey_address, hex_to_int, int_to_hex, Hash
...
class Storage(object):
    ...
    # convert between pesetacoin addresses and 20 bytes keys used for storage.
    def address_to_key(self, addr):
        return ptc_address_to_hash_160(addr)
    ...
    def listunspent(self, addr):
        key = self.address_to_key(addr)
        if key is None:
            raise BaseException('Invalid Pesetacoin address', addr)
    ...
    def write_undo_info(self, height, pesetacoind_height, undo_info):
        if height > pesetacoind_height - 100 or self.test_reorgs:
